...
int main(int argc, char *argv[]) {
    if (argc < 2) { // Verifica se o caminho foi passado
    ...
    if (S_ISDIR(path_stat.st_mode)) {
        // Processar todos os ficheiros .job num diretório
        process_directory(argv[1]);
    } else if (S_ISREG(path_stat.st_mode)) {
        // Processar um único ficheiro .job
...
void process_directory(const char *job_directory) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        // Ignorar entradas que não terminam em .job ou contêm .out
        if (!strstr(entry->d_name, ".job") || strstr(entry->d_name, ".out")) continue;
        ...
        if (input_len < 0 || input_len >= (int)sizeof(input_path)) {
            fprintf(stderr, "Error: input path too long: %s/%s\n", job_directory, entry->d_name);
            continue;
        }
        ...
        if (output_len < 0 || output_len >= (int)sizeof(output_path)) {
            fprintf(stderr, "Error: output path too long: %s/%s.out\n", job_directory, entry->d_name);
            continue;
        }
